...
def gen_geonorge_n50_url(cell, proj):
    zone = proj % 100
    base_url = f'https://nedlasting.geonorge.no/geonorge/Basisdata/N50RasterUTM{zone}/TIFF/'
...
def get_geonorge_n50_url(cell, proj):
    url = gen_geonorge_n50_url(cell, proj)
...
cells['url'] = cells.apply(lambda cell: gen_geonorge_n50_url(cell['n'], cell['source_proj']), axis=1)
print(cells.url.to_csv(index=False))
